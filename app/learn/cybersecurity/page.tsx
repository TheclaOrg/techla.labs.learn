...
import { UserTopicMastery, ProblemProgress, CyberCategory } from "@/types/learning";
...
export default function CybersecurityOverviewPage() {
  const [masteries] = useState<Record<string, UserTopicMastery>>(() =>
  ...
  const analytics = computeTrackAnalytics("cybersecurity", masteries, problemProgress);
  const personalized = generatePersonalizedPath(masteries, "cybersecurity");
  const nextTopic = personalized.currentTopic;
  const nextReason =
    personalized.reasoning[0]?.reason ||
    "Recommended next step based on cybersecurity prerequisite foundations.";

  const filteredTopics =
    selectedCategory === "All"
      ? cyberTopics
      : cyberTopics.filter((t) => t.category === selectedCategory);
      ...
      <div className="mx-auto max-w-7xl">
        ...
        <div className="flex flex-col justify-between gap-6 lg:flex-row lg:items-end">
          <div>
            <div className="inline-flex items-center gap-2 rounded-full border border-[#ff6a00]/20 bg-[#ff6a00]/5 px-3 py-1 text-xs font-semibold text-[#ff8533] mb-4">
              <Shield size={13} />
              CYBERSECURITY ROADMAP (36 NODES)
            </div>

            <h1 className="text-4xl font-bold tracking-[-.05em] sm:text-6xl text-white">
              Cybersecurity <span className="text-[#ff6a00]">&</span> Defensive Engineering
            </h1>

            <p className="mt-3 max-w-2xl text-sm sm:text-base text-white/50 leading-7">
              A comprehensive 12-module graph curriculum covering IT foundations, packet analysis, Linux/Windows internals, SOC operations, web exploitation, and cloud defense.
            </p>
          </div>
        ...
        <div className="mt-10">
          <TrackStatsHub
            ...
            diagnosticHref="/learn/cybersecurity/diagnostic"
          />
        </div>
        ...
        <div className="mt-10">
          <div className="rounded-2xl border border-[#ff6a00]/30 bg-[#0d0d0d] p-6 sm:p-7 flex flex-col justify-between shadow-[0_0_50px_rgba(255,106,0,0.06)]">
            <div>
              ...
              <div className="mt-3 flex flex-col sm:flex-row sm:items-baseline justify-between gap-2">
                <h2 className="text-2xl font-bold text-white">
                  {nextTopic?.title || "OSI & TCP/IP Network Models"}
                </h2>
            ...
            <div className="mt-6 pt-4 border-t border-white/[0.06] flex items-center justify-between">
              ...
              <Link
                href={`/learn/cybersecurity/${nextTopic?.slug || "osi-and-tcpip-models"}`}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
            <div>
              ...
              <h2 className="mt-2 text-2xl font-bold text-white">
                All 36 Cybersecurity Knowledge Nodes
              </h2>
            </div>
